document.addEventListener("DOMContentLoaded", () => {
  ...
  // Ajouter l'écouteur pour la mise à jour dynamique
  document.addEventListener("recentProductsUpdated", async () => {
    const recentProducts = JSON.parse(
      localStorage.getItem("recentProducts") || "[]"
    );
    if (recentProducts.length > 0) {
      ...
      if (productGrid) {
        await loadRecentProductsInline(recentProducts);
      } else {
        // Si la section n'existe pas encore, la charger
        location.reload();
      }
    }
  });
