...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    if (regex.test(prixValue)) {
      formData.append("price", prixValue);
    } else {
    ...
    formData.append("img", document.getElementById("productImage").files[0]); // Nouvelle image
    formData.append(
      "current_image",
    ...
    fetch("../public/index.php?api=produit&action=updateProduit", {
      ...
      .catch((error) => {
        ...
        alertContainer.innerHTML = `<div class="alert alert-danger">Une erreur est survenue</div>`;
      });
  });
